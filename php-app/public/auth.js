...
if (signupForm) {
  signupForm.addEventListener('submit', async (e) => {
    ...
    try {
      const { res, data } = await fetchJsonSafe(`${apiBase}/auth/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (res.ok && data.token) {
        clearReferral();
        handleLogin(data.token, data.user?.role, { showSuccess: true, successMessage: 'Registo bem sucedido! Redirecionando para o início...' });
      } else {
        showAppDialog(data.message || 'Erro no registo');
      }
    } catch (err) {
      showAppDialog(err?.message || 'Falha de ligação ao servidor no registo.');
    }
  });
}
...
if (signinForm) {
  signinForm.addEventListener('submit', async (e) => {
    ...
    try {
      const { res, data } = await fetchJsonSafe(`${apiBase}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (res.ok && data.token) {
        clearReferral();
        handleLogin(data.token, data.user?.role, { showSuccess: true, successMessage: 'Login efectuado com sucesso. A abrir o início...' });
      } else {
        showAppDialog(data.message || 'Erro no login');
      }
    } catch (err) {
      showAppDialog(err?.message || 'Falha de ligação ao servidor no login.');
    }
  });
}
